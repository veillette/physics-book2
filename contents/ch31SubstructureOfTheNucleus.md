...
the rest mass of a proton is converted entirely into energy, then

<div class="equation" >
 $$E=mc^{2}=\left(938.27 \text{MeV/}{c}^{2}\right){c}^{2}=938.27 \text{MeV} \text{.}  $$
</div>
...
All properties of a nucleus are determined by the number of protons and neutrons
it has. A specific combination of protons and neutrons is called a **nuclide**
and is a unique nucleus. The following notation is used to represent a
particular nuclide:
...
is the *number of neutrons* in a nucleus. In the notation for a nuclide, the
subscript $$N $$ is usually omitted. The symbol $$A $$ is defined as the number
of nucleons or the *total number of protons and neutrons*,

<div class="equation" >
 $$A=N+Z, $$
</div>

where $$A $$ is also called the **mass number**. This name for $$A $$ is
logical; the mass of an atom is nearly equal to the mass of its nucleus, since
electrons have so little mass. The mass of the nucleus turns out to be nearly
equal to the sum of the masses of the protons and neutrons in it, which is
proportional to $$A $$ . In this context, it is particularly convenient to
express masses in units of u. Both protons and neutrons have masses close to 1
u, and so the mass of an atom is close to $$A $$ u. For example, in an oxygen
nucleus with eight protons and eight neutrons, $$A=16 $$ , and its mass is 16 u.
...
There is some redundancy in the symbols $$A $$ , $$\text{X} $$ , $$Z $$ , and
$$N $$ . If the element $$\text{X} $$ is known, then $$Z $$ can be found in a
periodic table and is always the same for a given element. If both $$A $$ and
$$\text{X} $$ are known, then $$N $$ can also be determined (first find $$Z $$ ;
then, $$N=A-Z $$ ). Thus the simpler notation for nuclides is
...
tightly packed ball of nucleons, as illustrated in [[Figure 2]](#Figure2). These
nucleons have large kinetic energies and, thus, move rapidly in very close
contact. Nucleons can be separated by a large force, such as in a collision with
another nucleus, but resist strongly being pushed closer together. The most
compelling evidence that nucleons are closely packed in a nucleus is that the **radius of a nucleus**, $$r $$ , is found to be given approximately by
...
![This figure shows group of small green and blue spherical objects placed very close to each other forming a bigger sphere representing the nucleus. Blue spheres are labeled as protons and green spheres are labeled as neutrons.](../resources/Figure_31_03_02.jpg "A model of the nucleus.")
{: #Figure2}
...
(b) The density found here is so large as to cause disbelief. It is consistent
